...
public class Koalio extends ApplicationAdapter {
	...
	private void updateKoala (float deltaTime) {
        ...
        getshape1(startX, startY, endX, endY, tiles);
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape1");
                layer.setCell((int)tile.x, (int)tile.y, null);
                koala.MAX_VELOCITY=80;
            }
        }
        getshape2(startX, startY, endX, endY, tiles);
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape2");
                layer.setCell((int)tile.x, (int)tile.y, null);
                koala.MAX_VELOCITY=20;
            }
        }
        getshape3(startX, startY, endX, endY, tiles);
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape3");
                layer.setCell((int)tile.x, (int)tile.y, null);
                koala.JUMP_VELOCITY=60;
            }
        }
        getshape4(startX, startY, endX, endY, tiles);
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape4");
                layer.setCell((int)tile.x, (int)tile.y, null);
                koala.JUMP_VELOCITY=20;
            }
        }
        getshape5(startX, startY, endX, endY, tiles);
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape5");
                layer.setCell((int)tile.x, (int)tile.y, null);
                Koala.WIDTH=4;
                Koala.HEIGHT=4;
            }
    ...
    private void getshape1 (int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
        TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape1");
        rectPool.freeAll(tiles);
        tiles.clear();
        for (int y = startY; y <= endY; y++) {
    ...
    private void getshape2 (int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
        TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape2");
        rectPool.freeAll(tiles);
        tiles.clear();
        for (int y = startY; y <= endY; y++) {
    ...
    private void getshape3 (int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
        TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape3");
        rectPool.freeAll(tiles);
        tiles.clear();
        for (int y = startY; y <= endY; y++) {
    ...
    private void getshape4 (int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
        TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape4");
        rectPool.freeAll(tiles);
        tiles.clear();
        for (int y = startY; y <= endY; y++) {
    ...
    private void getshape5 (int startX, int startY, int endX, int endY, Array<Rectangle> tiles) {
        TiledMapTileLayer layer = (TiledMapTileLayer)map.getLayers().get("shape5");
        rectPool.freeAll(tiles);
        tiles.clear();
        for (int y = startY; y <= endY; y++) {
